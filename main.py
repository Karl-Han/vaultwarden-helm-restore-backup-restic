...
def common_options(command: Callable[..., Any]) -> Callable[..., Any]:
    options = [
        ...
        click.option(
            ...
            default="/data",
            show_default=True,
            help="Vaultwarden data directory managed by restic.",
        ),
        click.option(
            "--restic-repository",
...
@click.group()
def main() -> None:
    """Vaultwarden backup and restore helper."""
...
def backup(
    ...
    command_env = prepare_command_env(data_dir, rclone_config, password_file)
...
@click.option(
    "--snapshot",
    envvar="RESTIC_SNAPSHOT",
    default="latest",
    show_default=True,
    help="Restic snapshot or snapshot ID to restore.",
)
@click.option(
    "--restore-target",
    envvar="RESTIC_RESTORE_TARGET",
    default="/",
    show_default=True,
    help="Filesystem target passed to `restic restore --target`.",
)
def restore(
    namespace: str,
    statefulset: str,
